...
なお今回行ったシミュレーションに関わる全てのソースコードはGitHubレポジトリ(\url{https://github.com/akiratk0355/navigable-network-analysis})にて誰もが閲覧可能である.
  ...
  \subsection{シミュレーション結果}
   ...
   よって$D^3$-$DFS$はハブノードの存在によって恩恵を受けており, ハブの消失により確かに絶対的なパフォーマンスは低下するものの, 既存ルーティングアルゴリズムも同様にパフォーマンスが低下するため, 相対的には優れた性能を持ったアルゴリズムであることが分かる. これはEVNのヒューリスティックがgreedyルーティングのそれを包含しており, 次数のばらつきがない場合にEVNのヒューリスティックは単なる距離に基づいたものに近くなるだけだからである.
...
\section{結論と今後の方針}
本研究では主にFreenetで用いられる手法を概観し, Sandbergの埋め込み生成アルゴリズムSWAPの実データへの適用とそのlocal contact の存在率を算出することによりSWAPでは$D^2$-$DFS$によるpolylogルーティングが不可能である蓋然性が高いことを実証的に確認し, 実際未処理の実データを用いたルーティングシミュレーションを通じて先行研究に提示された結果よりも遙かに低いパフォーマンスを記録することを確認できた. 

その上でF2Fネットワークにおける分散ルーティングの効率性を向上させるために$D^3$-$DFS$を提案し, そのパフォーマンス評価を行った. より現実的なF2Fネットワークトポロジーに近いデータとハブノードの欠落という負の要素を伴ったデータ共に, シミュレーション結果において$D^3$-$DFS$が他を凌ぐことが確認できたため, Freenet等実際にデプロイされているF2Fネットワークの実装に活用できる可能性が大いにある. 特に現行のFreenetルーティングプロトコルである$D^2$-$DFS$よりも遙かに高い成功率と短い平均ホップ数を達成できた.

しかし例えばFreenetのルーティングプロトコルとして現実に組み込むためには, 以下の要素について十分な分析を行う必要性がある.
\begin{enumerate}[label={*}]
\item 更に良いヒューリスティックの設定方法はないか ?
\item 今回はデータの request 実験のみを行ったが , insert に対しても EVN を適用すべきか ?
\item より Freenet の実装に近い条件でのシミュレーション (Opennet モードとの共存 , churn あり , DoS アタックの存在 , etc)
\item 次数の情報を隣接ノードと共有することがセキュリティレベルに影響を与えるか ?
\item 埋め込みアルゴリズム自体の改良( e.g. Hyperbolic space model)
\end{enumerate}
