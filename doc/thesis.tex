...
\end{figure}
   ...
   次に未処理のWoTネットワークデータに次数上限を設け, ネットワーク全体でごく僅かな割合を占める少数のハブノードを取り除いたグラフに対して前節と同じシミュレーションを埋め込みの段階も含めて行った際の結果を図\ref{fig:clip_succ}, \ref{fig:clip_hops}に示す. ここで$k_{\max}$を次数上限とする. 2017年1月20日現在, FreenetのDarknetモードでは特に接続ノード数の上限はないが, Opennetモードの接続数上限が100であるため, 本研究でも$k_{\max}=100$を基準とし, さらに$k_{\max}$を減少させた場合についても検証を行った. 実際にデプロイされたF2Fオーバーレイネットワークにおけるノードは個人利用のマシンが大半であると想定されるため, 100という数字は同時接続数制限として妥当な数字と考えられる. 
   \begin{figure}[htbp]
   ...
   \end{figure}
   \begin{figure}[htbp]
    \centerline{\includegraphics[width=100mm]{../fig/cml_100clip.eps}}
    \caption{次数上限を設定したWeb of Trustネットワークにおける各ホップ数以下で成功したルーティング試行の割合 ($k_{\max}=100$)}
    \label{fig:cml_noclip}
   \end{figure}
\section{結論と今後の展望}
TBD
