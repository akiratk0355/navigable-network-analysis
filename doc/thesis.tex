...
%% 摘要 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\abstract{%
Friend-to-friend (F2F)ネットワークとは, 匿名性やプライバシーの保護を重視したP2Pネットワークであり, Freenet等の非中央集権的かつ安全なコミュニケーションツールの基礎となる概念である. 特にF2Fネットワーク上の各ノードは信頼のおける特定ノードとのみ通信を行うため, ネットワークトポロジーは現実世界の信頼関係ネットワークに対応し, かつ各ノードはネットワーク全体のトポロジーに関する情報を持たないという制約がある. そのような制約の中で如何にして分散的なルーティング(分散的なアルゴリズムによる任意のノード間の経路制御)の効率性を担保するか, ということがF2Fネットワークの利便性向上のための中心的課題である.
